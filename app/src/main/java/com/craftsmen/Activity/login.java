...
public class login extends Activity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        register.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                final String[] title = {"عامل","مستخدم"};

                AlertDialog.Builder choose = new AlertDialog.Builder(login.this);
                choose.setTitle("اختر نوع المستخدم");
               ...
               AlertDialog dialog =  choose.create() ;
                dialog.show();
                dialog.getWindow().getDecorView().setBackgroundColor(ContextCompat.getColor(getApplicationContext(),R.color.colorPrimary));
    ...
    void getUser(){
        ...
        StringRequest user_req = new StringRequest(Request.Method.POST, "http://192.168.56.1/craftsmen/login.php", new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                ...
                try {
                    ...
                    if (response.equals("Empty")){
                        Toast.makeText(getApplicationContext(),"اسم المستخدم او كلمة المرور غير صالحين",Toast.LENGTH_LONG).show();
                        ...
                        user_model.setU_gender(user_obj.getString("gender"));
                        user_model.setU_city(user_obj.getString("city"));
                        user_model.setLocation(user_obj.getString("location"));
                        user_model.setU_address(user_obj.getString("address"));
    ...
    boolean validate_user_name (){
        if (mob_num.getText().toString().trim().equals(null) || mob_num.getText().toString().trim().isEmpty()){
            user_name_container.setError("يجب ادخال قيمه");
            return false ;
        }
    ...
    boolean validate_user_pass(){
        if (password.getText().toString().trim().equals(null) || password.getText().toString().trim().isEmpty()){
            user_pass_container.setError("يجب ادخال قيمه");
            return false ;
        }
